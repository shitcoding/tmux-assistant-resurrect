...
in_env() {
	...
	env -u TMUX_ASSISTANT_RESURRECT_DIR -u XDG_RUNTIME_DIR -u XDG_STATE_HOME \
		"${assignments[@]}" bash -c "$snippet" 2>"${IN_ENV_ERR:-/dev/null}"
}
...
writer=$(env -u TMUX_ASSISTANT_RESURRECT_DIR \
	...
	bash -c "source '$LIB'; assistant_state_dir" 2>/dev/null)
...
out=$(env -u TMUX_ASSISTANT_RESURRECT_DIR -u HOME bash -c "source '$LIB'; assistant_state_dir" 2>&1)
...
if [ -d /var/folders ]; then
	out=$(env -u TMUX_ASSISTANT_RESURRECT_DIR -u XDG_RUNTIME_DIR -u XDG_STATE_HOME -u TMPDIR \
		...
		bash -c "source '$LIB'; legacy_assistant_state_dirs" 2>/dev/null)
...
if command -v node >/dev/null 2>&1; then
	...
	# they agree on how to spell a path. Under Git Bash they do not: node is a native
	# Windows build, so $HOME comes back as C:\Users\... where bash says /c/Users/...,
	# and a leading-slash override is resolved against the Git install root. Same
	# directory, different notation — and tmux has no Windows port to disagree on it.
	# The literal-source assertion above still runs everywhere, which is the half that
...
if [ -n "$mig_hazard" ]; then
	...
	# the save script chmods $STATE_DIR to 0700 at source time, which would undo
	# that, and locking it would stop the *second* file too — leaving nothing to
	# distinguish "the loop continued" from "the loop unwound". So put an
	# unwritable directory exactly where the first file's destination goes. A
	# plain directory at $dest would not fail (mv moves the file into it); one
	# with no write permission does, and only for that file.
	...
	# $dest exists, so the newest-wins branch runs first and would delete the
	# source as an older duplicate. Date it past the directory to reach the mv.
	touch -t 202701010000 "$err_legacy/claude-4270.json" 2>/dev/null
...
if [ -n "$(env -u TMUX_ASSISTANT_RESURRECT_DIR "HOME=$reap_home" "TMUX_RESURRECT_DIR=$SANDBOX/reap-resurrect" bash -c "source '$SAVE'; get_process_start_epoch $$" 2>/dev/null)" ]; then
	fixture "$reap_state/claude-$$.json" '{}'
...
out=$(env "TMUX_ASSISTANT_RESURRECT_DIR=$hint_dir" "HOME=$SANDBOX/h1" \
	...
	bash -c "source '$SAVE'; missing_session_hint claude 4242" 2>/dev/null)
...
out=$(env "TMUX_ASSISTANT_RESURRECT_DIR=$hint_dir" "HOME=$SANDBOX/h1" \
	"TMUX_RESURRECT_DIR=$SANDBOX/hint-resurrect" \
	bash -c "source '$SAVE'; missing_session_hint codex 4242" 2>/dev/null)
